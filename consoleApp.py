...
class ConsoleApp:
    _MESSAGE = "Нажмите на любую клавишу для продолжения..."
    _START_APP_MENU = [
        "1. Добавление книги",
        "2. Удаление книги",
        "3. Поиск книги",
    ...
    # Валидатор
    def validator(self,min_key,max_key,input_message="Введите номер : "):
        while True:
    ...
    def add_book_page(self)->None:
        ...
        while True:
            try:
                year = self.validator(1,int(datetime.now().year),"Введите год выпуска : ")
                break
            except ValueError:
                print("Введен некоректный год - Попробуйте снова ")

        status_int = self.validator(1,2,"Введите статус книги : (1 - 'в наличии' , 2 - 'выдана')")
    ...
    def search_book_page(self)->None:
        print("Если вы ищете книги по одному ключу оставляйте не нужные поля пустыми")
        title = input("Введите название книги : ")
        author = input("Введите автора книги : ")
        year = input("Введите год издания: ")
        l = self._library.search_book(
            title=title,
            author=author,
            year=year
        )
    ...
    def change_status_page(self)->None:
        ...
        new_status = self.validator(1,2,"Введите новый статус (1 - 'в наличии' , 2 - 'выдана') : ")
